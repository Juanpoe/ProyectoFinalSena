function ValidarHerramientaInsumo() {
    ...
    let NombreValido = Nombre.length >= 2 && Nombre.length <= 50 && /^[a-zA-Z0-9ñÑ\s]*$/.test(Nombre) && Nombre.trim() == Nombre;
    let DescripcionValido = Descripcion.length >= 2 && Descripcion.length <= 200 && Descripcion.trim() == Descripcion;
    let CantidadValido = /^[0-9]{1,7}$/.test(Cantidad);
...
function ValidarDescripcion(elemento) {
    ...
    if (inputValue.length < 2 || inputValue.length > 200) {
        // No cumple con las reglas
        elemento.style.borderColor = "red";
        errorSpan.textContent = "La descripción debe tener entre 2 y 200 caracteres";
    }
    else if (inputValue.trim() !== inputValue) {
        errorSpan.textContent = "La descripcion no puede iniciar ni terminar con un espacio"
    } else {
        // Cumple con las reglas
        elemento.style.borderColor = "green";
        errorSpan.textContent = "";
...
function ValidarCantidad(elemento) {
    ...
    if (!/^[0-9]{1,7}$/.test(inputValue)) {
        // No cumple con las reglas
        elemento.style.borderColor = "red";
        errorSpan.textContent = "La cantidad debe contener solo números y no tener una longitud mayor a 7 caracteres.";
    } else {
        // Cumple con las reglas
        elemento.style.borderColor = "green";
        errorSpan.textContent = "";
...
function ModalMensajeStock() {
    $.ajax({
        ...
        url: "../Controlador/HerramientaInsumo.php",
        data: {
            'Metodo': 'ModalMensajeStock'
        },
        success: function (data) {
            Swal.fire({
                icon: 'info',
                title: 'Mensaje de Stock',
                html: data.split('\n').join('<br><br>'), // Convertir saltos de línea en <br>
