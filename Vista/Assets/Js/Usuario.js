function ValidarCamposUsuario() {
    ...
    let nombreValido =  nombre.trim() == nombre && nombre.length >= 2 && nombre.length <= 20 && /^[a-zA-Z\u00D1ñÑ\s]+$/.test(nombre) && !/^\s/.test(nombre) && !/\s$/.test(nombre) && !/\s\s/.test(nombre);
    let apellidoValido = apellido.trim() == apellido && apellido.length >= 2 && apellido.length <= 20 && /^[a-zA-Z\u00D1ñÑ\s]+$/.test(apellido) && !/^\s/.test(apellido) && !/\s$/.test(apellido) && !/\s\s/.test(apellido);
    let documentoValido = /^[0-9]{6,20}$/.test(documento);
    let correoValido =  correo.length <= 50 && /^[\wñÑ.-]+@[a-zA-Z\d.-]+\.[a-zA-Z]{2,50}$/.test(correo);
    let contrasenaValido = contrasena.trim() == contrasena && contrasena.length >= 5 && contrasena.length <= 20;
    let telefonoValido = /^[0-9]{10,10}$/.test(telefono);
    let direccionValido = direccion.trim() == direccion && direccion.length >= 10 && direccion.length <= 50;
...
function ValidarNombreUsuario(elemento) {
    ...
    if (inputValue.trim() !== inputValue) {
        ...
        errorSpan.textContent = "El nombre no puede iniciar ni terminar con un espacio"
    }
    else if (inputValue.length < 2 || inputValue.length > 20 || !/^[a-zA-Z\u00D1ñÑ\s]+$/.test(inputValue) || /^\s/.test(inputValue) || /\s$/.test(inputValue) || /\s\s/.test(inputValue)) {
...
function ValidarApellidoUsuario(elemento) {
    ...
    if (inputValue.trim() !== inputValue) {
        elemento.style.borderColor = "red";
        errorSpan.textContent = "El apellido no puede iniciar ni terminar con un espacio"
    }
...
function ValidarCorreoUsuario(elemento) {
    ...
    if (inputValue.length > 50) {
        elemento.style.borderColor = "red";
        errorSpan.textContent = "El no debe tener mas de 50 caracteres"
    }
    else if (!/^[\wñÑ.-]+@[a-zA-Z\d.-]+\.[a-zA-Z]{2,50}$/.test(inputValue)) {
        // No cumple con las reglas
        elemento.style.borderColor = "red";
...
function ValidarContrasenaUsuario(elemento) {
    ...
    if (inputValue.trim() !== inputValue) {
        elemento.style.borderColor = "red";
        errorSpan.textContent = "La contraseña no puede iniciar ni terminar con un espacio"
    }
...
function ValidarDireccionUsuario(elemento) {
    ...
    if (inputValue.trim() !== inputValue) {
        elemento.style.borderColor = "red";
        errorSpan.textContent = "La direccion no puede iniciar ni terminar con un espacio"
    }
