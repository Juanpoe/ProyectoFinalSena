...
function ValidarCamposNovedad() {
    ...
    let PeticionValido = Peticion.length >= 5 && Peticion.length <= 100 && /^[a-zA-Z\s]+$/.test(Peticion) && !/^\s/.test(Peticion) && !/\s$/.test(Peticion) && !/\s\s/.test(Peticion)
    let DescripcionValido = Descripcion.length > 10 && Descripcion.length <= 300 && /^[a-zA-Z0-9\s]+$/.test(Descripcion) && !/^\s/.test(Descripcion) && !/\s$/.test(Descripcion) && !/\s\s/.test(Descripcion)
    ...
    let HoraFinValido = HoraFinHour >= 8 && HoraFinHour <= 17 && (HoraInicioHour <= HoraFinHour && HoraInicioMinutes <= HoraFinMinutes) && (HoraInicioHour != HoraFinHour && HoraInicioMinutes <= HoraFinMinutes)
...
function ValidaDescripcionNovedad(Elemento) {
    ...
    if (inputValue.length < 10 || inputValue.length > 300 || !/^[a-zA-Z0-9\s]+$/.test(inputValue) || /^\s/.test(inputValue) || /\s$/.test(inputValue) || /\s\s/.test(inputValue)) {
        // No cumple con las reglas
        Elemento.style.borderColor = "red";
        errorSpan.textContent = "La peticion debe tener entre 10 y 300 caracteres, no puede contener caracteres especiales, y solo puede tener espacios en medio del texto.";
    } else {
        // Cumple con las reglas
        Elemento.style.borderColor = "green";
        errorSpan.textContent = "";
...
function ValidateHoraInicio() {
    ...
    if (HoraInicioHour < 8 || HoraInicioHour > 17) {
...
function ValidateHoraFin() {
    ...
    if (HoraFinHour < 8 || HoraFinHour > 17) {
