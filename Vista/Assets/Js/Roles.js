function validarCamposRol() {
    ...
    let nombreValido = nombre.trim() == nombre && nombre.length >= 2 && nombre.length <= 20 && /^[a-zA-Z0-9ñÑ\s]+$/.test(nombre) && !/^\s/.test(nombre) && !/\s$/.test(nombre) && !/\s\s/.test(nombre);
...
function ValidarNombreRol(elemento) {
    ...
    if (inputValue.trim() !== inputValue || inputValue.length < 2 || inputValue.length > 20 || !/^[a-zA-Z0-9ñÑ\s]+$/.test(inputValue) || /^\s/.test(inputValue) || /\s$/.test(inputValue) || /\s\s/.test(inputValue)) {
